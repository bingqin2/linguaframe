...
The selected job view also includes a `Narration workspace` panel. Use it to add multiple start/end time ranges, enter explanatory narration text, choose voices, save the workspace, generate a timed `narration-audio.mp3` bed, and then generate `narrated-video.mp4`. Narration audio and narrated video are separate artifact types, `NARRATION_AUDIO` and `NARRATED_VIDEO`; they do not replace `DUBBING_AUDIO`, `DUBBED_VIDEO`, burned-video outputs, generated subtitles, or reviewed subtitle artifacts. Narrated-video export preserves the selected base video's original audio and ducks it to `0.35` during narration windows while mixing in narration audio. Waveform editing, drag/drop timeline editing, and adjustable mix controls remain future work.
