...
The selected job view also includes a `Media delivery` panel when playable outputs exist. It renders `DUBBING_AUDIO`, generated `BURNED_VIDEO`, and reviewed `REVIEWED_BURNED_VIDEO` as separate outputs with browser players, direct download links, content type, size, short SHA-256 hash, and generated/reused cache state. Reviewed burned video stays separate from the generated burned video so human-reviewed handoff media does not overwrite pipeline audit artifacts.
