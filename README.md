...
Use the browser `Demo session cost control` panel before another paid demo run. It calls `GET /api/operator/demo-session-cost-control-board`, combines recent model usage, owner daily estimated spend, budget guard posture, failed-call rate, top jobs, top operations, and a safe next action, and is summarized in the `Demo session command center`. The terminal equivalent is `scripts/demo/demo-session-cost-control-board.sh`, which writes JSON and Markdown under `/tmp/linguaframe-demo/demo-session-cost-control-board/` and exits non-zero when the board is `BLOCKED` unless `LINGUAFRAME_DEMO_SESSION_COST_CONTROL_BOARD_REPORT_ONLY=true`. The model usage ledger remains the raw spend/latency evidence; the cost-control board is the go/no-go safety surface. Both use local estimates, while provider billing remains the source of truth.
...
Uploads may also include an optional narration quick script to seed the job's narration workspace at creation time. Use `START-END | VOICE | TEXT`, for example `00:15-00:28 | demo-voice | Explain this moment.` or `00:55-01:10 || Inherit the default voice.`. Invalid rows block upload before source storage and dispatch. Valid rows only create editable narration workspace rows; upload does not call TTS providers, OpenAI, FFmpeg, or generate narration artifacts. Demo scripts accept `LINGUAFRAME_DEMO_NARRATION_SCRIPT=$'00:15-00:28 | demo-voice | Explain this moment.'` or `LINGUAFRAME_DEMO_NARRATION_SCRIPT_FILE=/path/to/narration-script.txt`.
