...
For explanatory voiceover, open a completed job in the browser and use `Narration workspace`. Add or import time-coded rows, then edit windows either through the table inputs, the proportional `Timeline workbench` bars, or `Narration editing commands`. Select a row to duplicate it, split it at the preview playhead, merge it with the next row, or insert a blank row for new commentary. Select a bar, use ArrowLeft/ArrowRight to move it by 0.25 seconds, Shift+ArrowLeft/Right to resize the end, or Alt+ArrowLeft/Right to resize the start. Use `Narration waveform overview` to inspect metadata-derived density/gaps and scrub the preview locally before saving. Use `Narration draft history` to review added/removed/timing/text/voice changes, undo or redo local edits, and revert the browser draft to the last saved workspace. Use `Narration TTS preview` on the selected row to synthesize a transient audio preview for the current unsaved text and voice before saving or generating the full narration bed. Segment preview can call the configured TTS provider and may consume OpenAI credits, but it does not save rows, create artifacts, write object storage, update evidence, or generate video. Timeline edits, waveform scrubbing, editing commands, undo, redo, and revert stay local until `Save narration`; they do not call OpenAI, generate audio/video, create artifacts, or save rows. Draft history resets after a successful save response or workspace reload. Inserted blank rows block save until required fields are filled. After validation passes, save narration, optionally generate narration audio/video, or run render preflight before the one-click narration demo render.
