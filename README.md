...
The selected job view also includes a `Narration workspace` panel. Use it to add multiple start/end time ranges, enter explanatory narration text, choose voices, save the workspace, and generate `narration-audio.mp3`. Narration audio is a separate artifact type, `NARRATION_AUDIO`; it does not replace `DUBBING_AUDIO`, `DUBBED_VIDEO`, burned-video outputs, generated subtitles, or reviewed subtitle artifacts. This slice does not mux narration back into a video; narrated-video export remains future work.
