...
The selected job view also includes a `Narration workspace` panel. Use it to add multiple start/end time ranges, enter explanatory narration text, choose voices, save the workspace, generate `narration-audio.mp3`, and then generate `narrated-video.mp4`. Narration audio and narrated video are separate artifact types, `NARRATION_AUDIO` and `NARRATED_VIDEO`; they do not replace `DUBBING_AUDIO`, `DUBBED_VIDEO`, burned-video outputs, generated subtitles, or reviewed subtitle artifacts. This slice uses FFmpeg audio replacement for a standalone narrated video. Audio ducking, multi-track mixing, waveform editing, and drag/drop timeline editing remain future work.
