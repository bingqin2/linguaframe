...
The browser `Demo sample media` panel and `scripts/demo/demo-sample-media-catalog.sh` expose the same list with attribution, configured local sample availability, upload duration limit, and safe demo commands. The adjacent browser `Demo run launcher` panel and `scripts/demo/demo-run-launcher.sh` connect that sample choice to the recommended `tears-showcase` full-demo command, readiness gates, and expected evidence files. After a job completes, use the browser `Demo completion certificate` panel or `LINGUAFRAME_DEMO_JOB_ID=<job-id> scripts/demo/demo-completion-certificate.sh` for one metadata-only proof of completion, handoff readiness, reproducibility, evidence links, and next action. Use the browser `Demo replay card` panel or `LINGUAFRAME_DEMO_JOB_ID=<job-id> scripts/demo/demo-replay-card.sh` to export the job's settings, replay commands, baseline comparison command, and safe package links. These surfaces are read-only: they do not download media, edit `.env`, upload files, start Docker, or call OpenAI.
