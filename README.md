...
The selected job view also includes a `Narration workspace` panel. Use it to add multiple start/end time ranges, enter explanatory narration text, choose voices, save the workspace, tune mix settings, generate a timed `narration-audio.mp3` bed, and then generate `narrated-video.mp4`. Narration audio and narrated video are separate artifact types, `NARRATION_AUDIO` and `NARRATED_VIDEO`; they do not replace `DUBBING_AUDIO`, `DUBBED_VIDEO`, burned-video outputs, generated subtitles, or reviewed subtitle artifacts. Narrated-video export preserves the selected base video's original audio, ducks it during narration windows, applies narration volume, and adds short fades. Defaults are `duckingVolume=0.35`, `narrationVolume=1.00`, and `fadeDurationMs=250`; allowed ranges are `0.00-1.00`, `0.00-2.00`, and `0-5000`. Evidence reports whether values came from defaults or saved operator settings. Waveform editing, drag/drop timeline editing, and full multitrack editing remain future work.
