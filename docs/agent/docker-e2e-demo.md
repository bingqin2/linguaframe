...
Check the `Subtitle draft editor` panel when target subtitles exist. Edit one draft row, save it, verify the saved/unsaved counters update, use corrected JSON/SRT/VTT export links, reset unsaved edits, and clear the saved draft. Corrected subtitle exports use the saved draft overlay, while generated subtitle artifacts, TTS audio, and burned video stay unchanged.
