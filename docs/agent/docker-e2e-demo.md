...
Before uploading, check the browser `Demo runbook`, `Live checks`, and upload form `Validate file` result. The runbook shows the one-command startup path, short/cache/full E2E commands, local frontend and backend health URLs, sample-media guidance, and current runtime constraints such as upload duration, provider modes, budget guard, and subtitle burn-in state. The live-check panel should show database, Redis, RabbitMQ, MinIO, and FFmpeg as `UP`, `DOWN`, or `SKIPPED`. The upload form can apply a `Demo profile` such as `quick-baseline`, `tears-showcase`, or `concise-review` before manual edits; the selected profile should appear in job detail, delivery handoff, evidence, demo session reports, and terminal Markdown summaries. The upload validation panel should show the selected file's validation code, message, size, duration, and configured limits before any job is created.
