...
Check the `Narration studio` panel first when you need explanatory voiceover. It is the guided entry point for upload-seeded or manually authored narration: author rows, preview TTS, render custom narration, review playback, package delivery, and final handoff. It links to existing specialized panels and stays read-only. Terminal equivalent:

```bash
LINGUAFRAME_DEMO_JOB_ID=<job-id> scripts/demo/narration-studio.sh
```

Expected output includes `narrationStudioStatus`, `narrationStudioPhase`, row/character counts, audio/video readiness, one `narrationStudioStep=...` line per step, and safe links. It must not call OpenAI, call TTS providers, run FFmpeg, save narration rows, create artifacts, print narration text, print reviewer notes, expose object keys, expose local paths, or include media bytes.

Use the `Narration workspace` panel for the actual editing controls. If the video was uploaded with a narration script, the `Upload narration launchpad` shows seeded row count, character count, selected row, voice summary, scene-board status, explicit next actions, safe links, and `Download launchpad Markdown` without printing narration text. Then add or edit rows, preview TTS, save the workspace, run custom render preflight, acknowledge provider/video cost, render, review playback, and export the narration delivery package.
...
If the monitor shows a job stuck before worker pickup or at a stale processing stage, open the `Stuck job recovery` panel or run `LINGUAFRAME_DEMO_JOB_ID=<job-id> scripts/demo/stuck-job-recovery.sh`. It should call `GET /api/jobs/{jobId}/stuck-job-recovery`, download `stuck-job-recovery.json` and `.md`, print `stuckJobRecovery*` summary lines, and exit non-zero for blocked/stale states unless `LINGUAFRAME_STUCK_JOB_RECOVERY_REPORT_ONLY=true`. Confirmed shell actions require both `LINGUAFRAME_STUCK_JOB_RECOVERY_ACTION` and matching `LINGUAFRAME_STUCK_JOB_RECOVERY_CONFIRM`. Use this cockpit before manual database edits; it reuses existing retry/cancel semantics and only requeues a fresh dispatch event for `QUEUED_STALE_DISPATCH`.

Use the `Demo session command center` first for run-day recovery. It should call `GET /api/operator/demo-session-command-center`, show recovery status and recover-now/watch/review/ready counts, become `BLOCKED` when the session recovery board has recover-now rows, and link to the full recovery board Markdown before you continue the demo. The terminal equivalent is `scripts/demo/demo-session-command-center.sh`, which prints `demoSessionCommandCenterRecovery*` summary lines.

Use the `Demo session recovery board` panel when you need the operator-level recovery drill-down across recent jobs. It should call `GET /api/operator/demo-session-recovery-board`, show overall `READY`, `ATTENTION`, `BLOCKED`, or `EMPTY` status, count recover-now/watch/ready/review/no-action rows, and link each job to existing per-job evidence or recovery routes without executing an action. The terminal equivalent is `scripts/demo/demo-session-recovery-board.sh`, which writes `demo-session-recovery-board.json` and `.md` under `/tmp/linguaframe-demo/demo-session-recovery-board/`, prints `demoSessionRecoveryBoard*` summary lines, and exits non-zero when recover-now rows exist unless `LINGUAFRAME_DEMO_SESSION_RECOVERY_BOARD_REPORT_ONLY=true`. `scripts/demo/demo-session-evidence-package.sh` also confirms the session ZIP contains `recovery-board.json` and `recovery-board.md`.

Use the `Private demo delivery receipt` panel as the final operator receipt after selecting or completing the recommended run. It should call `GET /api/operator/private-demo/delivery-receipt`, show overall status, selected or recommended job, delivery checks, safe next action, evidence links, and package entries. The terminal equivalent is `scripts/demo/private-demo-delivery-receipt.sh`, which writes JSON, Markdown, and ZIP files under `/tmp/linguaframe-demo/private-demo-delivery-receipt/`, prints `privateDemoDeliveryReceipt*` summary lines, and exits non-zero only when the receipt is `BLOCKED` unless `LINGUAFRAME_PRIVATE_DEMO_DELIVERY_RECEIPT_REPORT_ONLY=true`. The receipt ZIP must contain metadata-only `manifest.json`, `private-demo-delivery-receipt.json`, `private-demo-delivery-receipt.md`, `run-archive.json`, `command-center.json`, and `README.md`, and must link existing packages instead of nesting ZIP binaries.
...
Use the demo acceptance gate as the final go/no-go check before presenting. `GET /api/jobs/{jobId}/demo-acceptance-gate`, the browser `Demo acceptance gate` panel, `LINGUAFRAME_DEMO_JOB_ID=<job-id> scripts/demo/demo-acceptance-gate.sh`, and the full Tears script output `demo-acceptance-gate.json` should summarize required checks, warning checks, safe evidence metrics, package links, narration playback resolution status/counts, narration delivery package readiness/audio/video/entry evidence, recommended next action, and `Resolution runbook` steps as `READY`, `ATTENTION`, or `BLOCKED`. If saved narration rows have unresolved playback review decisions, the required `NARRATION_PLAYBACK_RESOLVED` check blocks final acceptance and the runbook should point to `scripts/demo/narration-playback-review-resolution.sh` plus `/api/jobs/{jobId}/narration-playback-review/resolution`. The browser `Acceptance recovery` panel should then show the blocked gate status, unresolved/rerender counts, render/playback review status, narration audio/video readiness, a focus action for unresolved rows, refresh actions, and a `Recovery handoff` area. `GET /api/jobs/{jobId}/narration-recovery-handoff`, its Markdown/ZIP download routes, `LINGUAFRAME_DEMO_JOB_ID=<job-id> scripts/demo/narration-recovery-handoff.sh`, and the full Tears script output should provide the same metadata-only recovery handoff package with acceptance gate, playback resolution, package inventory, safe links, and remediation steps. After playback resolution is ready, use the browser `Narration delivery package`, `GET /api/jobs/{jobId}/narration-delivery-package`, its Markdown/ZIP download routes, or `LINGUAFRAME_DEMO_JOB_ID=<job-id> scripts/demo/narration-delivery-package.sh` to export final narration delivery status, artifact links, checks, and package inventory. The full Tears script exports this package under `narration-delivery-package/`, then refreshes acceptance gate, reviewer workspace, and handoff portal outputs so final handoff packages include current narration delivery status. It should not include media bytes, raw transcript text, raw subtitles, corrected draft text, raw narration text outside the explicit script package, reviewer note bodies, object keys, local paths, demo tokens, provider payloads, credentials, or API keys.
...
LINGUAFRAME_DEMO_JOB_ID=<job-id> scripts/demo/narration-delivery-package.sh
```
