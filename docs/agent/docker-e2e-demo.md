...
Use the browser `Demo sample media` panel or `scripts/demo/demo-sample-media-catalog.sh` before the full run to verify the recommended Tears sample status and duration-limit guidance without exposing the full local path.
