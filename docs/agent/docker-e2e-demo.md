...
Use the `Demo session recovery board` panel when you need the operator-level recovery view across recent jobs. It should call `GET /api/operator/demo-session-recovery-board`, show overall `READY`, `ATTENTION`, `BLOCKED`, or `EMPTY` status, count recover-now/watch/ready/review/no-action rows, and link each job to existing per-job evidence or recovery routes without executing an action. The terminal equivalent is `scripts/demo/demo-session-recovery-board.sh`, which writes `demo-session-recovery-board.json` and `.md` under `/tmp/linguaframe-demo/demo-session-recovery-board/`, prints `demoSessionRecoveryBoard*` summary lines, and exits non-zero when recover-now rows exist unless `LINGUAFRAME_DEMO_SESSION_RECOVERY_BOARD_REPORT_ONLY=true`.
