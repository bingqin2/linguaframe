...
Check the `Narration workspace` panel when you need explanatory voiceover. Add or edit time-coded rows, inspect the `Timeline workbench` proportional bars, use the selected-segment inspector for exact window, duration, voice, and character count, save narration, save mix settings, generate narration audio, generate narrated video, then refresh or download narration evidence. Gaps are allowed and mean intentional silence between narration windows; overlapping or invalid rows block save/generate actions until fixed. Evidence refresh and Markdown/ZIP downloads remain available so blocked states can still be diagnosed safely.
