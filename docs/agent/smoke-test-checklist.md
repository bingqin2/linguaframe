...
- Narration evidence reports status, segment count, character count, total timeline duration, audio readiness, `audioLayout`, `timeAligned`, narrated-video readiness, `mixMode`, `duckingVolume`, `narrationVolume`, `fadeDurationMs`, `mixSettingsSource`, safe links, and ZIP entries only. It must not include narration script bodies, transcript text, subtitle text, object keys, local paths, demo tokens, provider payloads, credentials, or media bytes.
