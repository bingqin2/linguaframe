...
- The `Narration workspace` panel can add rows, edit start/end/text/voice, save valid non-overlapping segments, clear the workspace, generate narration audio, refresh evidence, and download narration evidence Markdown/ZIP.
- Generated narration audio appears as `NARRATION_AUDIO` in artifacts and as a playable `Narration audio` media-delivery card. It must not replace `DUBBING_AUDIO`, `DUBBED_VIDEO`, `BURNED_VIDEO`, or reviewed subtitle artifacts.
- Narration evidence reports status, segment count, character count, total timeline duration, audio readiness, safe links, and ZIP entries only. It must not include narration script bodies, transcript text, subtitle text, object keys, local paths, demo tokens, provider payloads, credentials, or media bytes.
