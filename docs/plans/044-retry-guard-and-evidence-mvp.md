...
- [ ] Commit as `Add retry guard and evidence`.
- [ ] Merge `retry-guard-and-evidence-mvp` back to `main`.
- [ ] Run post-merge focused validation: backend focused command and `cd frontend && npm run test:run -- App`.
