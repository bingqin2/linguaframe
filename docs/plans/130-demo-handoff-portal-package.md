...
- [ ] Write failing service tests for `READY`, `ATTENTION`, `BLOCKED`, missing job propagation, static HTML entry presence, ZIP entries, and unsafe marker exclusion.
- [ ] Implement portal VO records with `jobId`, `videoId`, `generatedAt`, `overallStatus`, `phase`, `headline`, `recommendedNextAction`, `completedAt`, `targetLanguage`, `demoProfileId`, `checks`, `sections`, `safeLinks`, `packageEntries`, and `safetyNotes`.
- [ ] Compose portal data from `LocalizationJobQueryService`, `DemoReviewerWorkspaceService`, `DemoAcceptanceGateService`, `DemoCompletionCertificateService`, `DemoRunSnapshotService`, `DemoShareSheetService`, `DemoRunMonitorService`, `DeliveryManifestService`, and `OpenAiSmokeProofService`.
- [ ] Derive status using required checks for completed job, reviewer workspace, safe evidence links, portal package entries, and required package routes.
- [ ] Render Markdown with summary, checks, sections, safe links, package inventory, and safety notes.
- [ ] Render static `index.html` with escaped text only, no inline raw transcript/subtitle bodies, and no external scripts.
- [ ] Build ZIP with `index.html`, `manifest.json`, `handoff-portal.md`, `reviewer-workspace.json`, `README.md`, and safe JSON snapshots for acceptance/certificate/share-sheet/run-monitor when available.
- [ ] Run `mvn -pl LinguaFrame test -Dtest=DemoHandoffPortalServiceTests`.
...
- [ ] Add JSON, Markdown, and ZIP endpoints beside the reviewer workspace routes.
- [ ] Use `Content-Disposition` filenames `linguaframe-job-{jobId}-demo-handoff-portal.md` and `.zip`.
- [ ] Add routes to the runtime required-route contract.
- [ ] Add MockMvc coverage for JSON status, Markdown headers/body safety, ZIP entries including `index.html`, auth behavior, runtime route contract, and unsafe marker exclusion.
- [ ] Run `mvn -pl LinguaFrame test -Dtest=DemoHandoffPortalServiceTests,LocalizationJobControllerTests,RuntimeDependencyControllerTests`.
...
- [ ] Add TypeScript types matching the backend portal VO.
- [ ] Add API helpers using the existing demo-token and bearer-token behavior.
- [ ] Load the portal whenever selected job evidence loads: upload completion, manual open, retry/cancel result, recent/history/dashboard selection, polling, and SSE refresh.
- [ ] Add a `Demo handoff portal` panel near `Demo reviewer workspace` with status, phase, headline, required checks, optional checks, package entries, safe links, refresh, Markdown download, and ZIP download.
- [ ] Keep selected-job details usable if portal loading fails.
...
- [ ] Add helper functions to download portal JSON, Markdown, and ZIP.
- [ ] Add `print_demo_handoff_portal_summary_file` that validates ZIP entries, checks unsafe markers, and prints `demoHandoffPortal*` summary keys.
- [ ] Implement the standalone script to write `demo-handoff-portal.json`, `.md`, and `.zip`; exit non-zero on `BLOCKED` unless `LINGUAFRAME_DEMO_HANDOFF_PORTAL_REPORT_ONLY=true`.
- [ ] Extend deterministic, OpenAI smoke, and full Tears E2E scripts to export the portal after job completion.
- [ ] Document when to use portal versus reviewer workspace, demo snapshot, demo run package, evidence closure, acceptance gate, completion certificate, and OpenAI smoke proof.
- [ ] Update smoke checklist with browser panel expectations, script output, ZIP entries, `index.html`, and safety exclusions.
- [ ] Run `bash -n scripts/demo/demo-handoff-portal.sh scripts/demo/docker-e2e-success.sh scripts/demo/docker-e2e-openai-smoke.sh scripts/demo/docker-e2e-tears-of-steel-full.sh scripts/demo/lib/linguaframe-demo.sh`.
...
- [ ] Run full safety checks:
  ...
  `npm test -- --run`
  `git diff --check`
- [ ] Commit as `Add demo handoff portal package`.
