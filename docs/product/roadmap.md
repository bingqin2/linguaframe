...
- Narration one-click render package. Status: implemented with `POST /api/jobs/{jobId}/narration-demo/render`, a browser `Render narration demo` panel with replacement and paid-provider acknowledgements, terminal `scripts/demo/narration-demo-render.sh`, and full Tears `LINGUAFRAME_RENDER_NARRATION_DEMO=true` integration. Render reuses preset apply, narration audio, narrated-video, script package, evidence, and artifact services; audio-only and partial-video-failure outcomes remain explicit.
