...
- Time-coded custom narration segments. Status: implemented with backend persistence, workspace APIs, provider-aware voice preset selection, React narration scene board, React narration timeline workbench with move/resize editing, local duplicate/split/merge/insert row commands, local timing assistant gap/overlap detection and fix actions, quick script paste import/export for `START-END | VOICE | TEXT` rows, in-memory draft history with undo/redo/revert, decoded waveform with metadata fallback, persistent waveform artifacts, mix automation curves over effective ducking/narration/fade values, persistent mix keyframes, local preview against narrated/burned/source video, transient voice-preset audition and selected-segment TTS preview for unsaved text/voice, jump/play-window/scrub controls, visible preview and waveform playheads, numeric job-level mix controls, optional per-segment ducking/narration/fade overrides, per-segment TTS mixed into a timed `NARRATION_AUDIO` bed, standalone `NARRATED_VIDEO` export that preserves original/base audio and applies saved job defaults plus segment overrides during narration windows, metadata-only narration scene-board JSON/Markdown export, metadata-only narration evidence, read-only narration render review cue sheet with JSON/Markdown downloads, persisted segment-level playback review decisions with private note input and metadata-only JSON/Markdown exports, playback resolution gate for unresolved review rows with safe focus actions, browser acceptance recovery for narration-blocked final gates, downloadable recovery handoff JSON/Markdown/ZIP, read-only narration delivery package JSON/Markdown/ZIP with safe generated-media links, final acceptance-gate delivery package evidence/links, reviewer workspace and handoff portal delivery package summaries, and required final acceptance-gate blocking, explicit narration script package export/import for workspace reuse, one-click demo-preset render orchestration, custom saved-workspace render preflight/render/report orchestration, terminal scene-board export, terminal timing-assistant preflight, terminal voice-audition and segment-preview MP3 export, and terminal evidence/package/custom-render/demo-render/playback-review/resolution/recovery-handoff/delivery exports. The workbench shows scene-board coverage, saved metadata with local draft text, segment rail, timeline board, selected-row inspector, span, covered time, gaps, readiness, proportional segment bars, timing assistant issue/fix status, draft added/removed/timing/text/voice change counts, decoded or fallback waveform buckets, waveform artifact/cache/source metadata, quick-script row/error previews, copy/download quick-script export text, selected-segment diagnostics, default/explicit voice preset state, selected-row mix override controls, mix automation override/inheritance state, voice audition status, TTS preview status, custom render preflight/results, render step status, render review checks, playback review counts, playback resolution counts, acceptance recovery status, recovery handoff package inventory, delivery package readiness, script package checks, and local-only preview/editing state.
- Guided narration studio. Status: implemented as `GET /api/jobs/{jobId}/narration-studio`, a browser `Narration studio` workbench, and `scripts/demo/narration-studio.sh` for the metadata-only operator path from authoring through final handoff.
...
- Lip sync.
- Full nonlinear video editing, multitrack automation curves, voice cloning, uploaded reference audio, or replacing existing subtitle review artifacts.
...
- Operators can add multiple time-coded explanatory voiceover segments manually or through quick script paste import, copy/download the current local draft as reusable quick script text, audition configured voice presets, apply a voice to selected/all local draft rows, duplicate/split/merge/insert draft rows, inspect intentional gaps, use the timing assistant to close gaps, resolve overlaps, or normalize row order locally, move or resize windows before generation, inspect local undo/redo/revert history and unsaved-change counts, inspect decoded waveform source/buckets with metadata fallback, reuse persistent waveform artifacts by source hash and bucket count, inspect mix automation curves, add persistent mix keyframes, and apply/clear local overrides, preview and scrub selected windows against existing media with visible playheads, preview selected-row TTS before saving, run read-only terminal timing preflight, export a read-only render review cue sheet, and produce separate timed narration audio plus a standalone narrated video with configurable original-audio ducking, narration gain, and fades without disrupting the current localization pipeline. Future slices can add deeper multitrack editing.
...
- Narration demo preset package. Status: implemented with read-only profile-linked preset catalog endpoints, `POST /api/jobs/{jobId}/narration-demo-preset/apply`, a browser `Demo narration preset` panel, terminal `scripts/demo/narration-demo-preset.sh`, and optional full Tears preset application before narration evidence export. Preset apply imports operator-authored script rows and mix settings, requires replace confirmation, and keeps audio/video generation separate.
- Narration one-click render package. Status: implemented with read-only `POST /api/jobs/{jobId}/narration-demo/render/preflight`, `POST /api/jobs/{jobId}/narration-demo/render`, a browser `Render narration demo` panel with preflight, replacement, and paid-provider acknowledgements, terminal `scripts/demo/narration-demo-render-preflight.sh` plus `scripts/demo/narration-demo-render.sh`, and full Tears `LINGUAFRAME_RENDER_NARRATION_DEMO=true` integration. Render reuses preset apply, narration audio, narrated-video, script package, evidence, and artifact services; audio-only, blocked-preflight, and partial-video-failure outcomes remain explicit.
- Custom narration render console. Status: implemented with `POST /api/jobs/{jobId}/custom-narration-render/preflight`, `POST /api/jobs/{jobId}/custom-narration-render`, Markdown report download, browser `Render custom narration`, terminal `scripts/demo/custom-narration-render.sh`, optional seeded short/full E2E integration through `LINGUAFRAME_RENDER_CUSTOM_NARRATION=true`, and final handoff visibility in acceptance gate, reviewer workspace, handoff portal, evidence closure, and full Tears terminal summaries. This path renders the saved operator-authored workspace only; it does not apply demo presets or replace rows.
...
- Demo session recovery board. Status: implemented with `GET /api/operator/demo-session-recovery-board`, Markdown download, a browser run-day recovery panel, and `scripts/demo/demo-session-recovery-board.sh` for metadata-only session-level triage across recent jobs while reusing existing per-job recovery and evidence routes. The board is also integrated into the demo session command center and session evidence package so recover-now rows block the top-level run-day flow and export `recovery-board.json` plus `recovery-board.md`.
- Session narration production board. Status: implemented with `GET /api/operator/session-narration-production-board`, Markdown download, a browser run-day narration production panel, and `scripts/demo/session-narration-production-board.sh` for metadata-only triage across recent jobs into ready, review, render, authoring, blocked, and not-applicable rows while reusing existing narration scene-board, render-review, playback-resolution, delivery-package, acceptance-gate, and recovery routes. The board is also integrated into the demo session command center and session evidence package so blocked narration rows block the top-level run-day flow and export `narration-production-board.json` plus `narration-production-board.md`.
- Demo session cost-control board. Status: implemented with `GET /api/operator/demo-session-cost-control-board`, Markdown download, a browser run-day cost safety panel, and `scripts/demo/demo-session-cost-control-board.sh` for metadata-only budget posture, owner daily spend, recent estimated spend, failed-call rate, top jobs, top operations, and safe next action. The board is also integrated into the demo session command center and session evidence package so blocked cost posture blocks the top-level run-day flow and exports `cost-control-board.json` plus `cost-control-board.md`.
- Retry button.
- Cost summary. Status: implemented through job usage summaries, the operator model usage ledger, and the session cost-control board. Ledger views provide raw model-call spend and latency evidence; the cost-control board provides the run-day go/no-go safety posture using local estimates.
...
- Private demo delivery receipt. Status: implemented through `GET /api/operator/private-demo/delivery-receipt`, Markdown/ZIP downloads, the React `Private demo delivery receipt` panel, and `scripts/demo/private-demo-delivery-receipt.sh` as the final metadata-only proof of selected/recommended run, readiness, command center state, safe package routes, reviewer workspace, handoff portal, evidence closure, OpenAI proof, and AI audit links.
