...
- Time-coded custom narration segments. Status: implemented with backend persistence, workspace APIs, provider-aware voice preset selection, React narration timeline workbench with move/resize editing, local preview against narrated/burned/source video, jump/play-window controls, a visible preview playhead, numeric mix controls, per-segment TTS mixed into a timed `NARRATION_AUDIO` bed, standalone `NARRATED_VIDEO` export that preserves original/base audio and applies saved ducking, narration gain, and fades during narration windows, metadata-only narration evidence, explicit narration script package export/import for workspace reuse, one-click demo render orchestration, and terminal evidence/package/render exports. The workbench shows span, covered time, gaps, readiness, proportional segment bars, selected-segment diagnostics, default/explicit voice preset state, render step status, script package checks, and local-only preview state.
