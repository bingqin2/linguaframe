...
- Time-coded custom narration segments. Status: implemented with backend persistence, workspace APIs, provider-aware voice preset selection, React narration timeline workbench with move/resize editing, local duplicate/split/merge/insert row commands, quick script paste import/export for `START-END | VOICE | TEXT` rows, in-memory draft history with undo/redo/revert, metadata-derived waveform overview, local preview against narrated/burned/source video, transient voice-preset audition and selected-segment TTS preview for unsaved text/voice, jump/play-window/scrub controls, visible preview and waveform playheads, numeric mix controls, per-segment TTS mixed into a timed `NARRATION_AUDIO` bed, standalone `NARRATED_VIDEO` export that preserves original/base audio and applies saved ducking, narration gain, and fades during narration windows, metadata-only narration evidence, explicit narration script package export/import for workspace reuse, one-click demo render orchestration, terminal voice-audition and segment-preview MP3 export, and terminal evidence/package/render exports. The workbench shows span, covered time, gaps, readiness, proportional segment bars, draft added/removed/timing/text/voice change counts, waveform density/gap buckets, quick-script row/error previews, copy/download quick-script export text, selected-segment diagnostics, default/explicit voice preset state, voice audition status, TTS preview status, render step status, script package checks, and local-only preview/editing state.
