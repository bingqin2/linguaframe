...
- TTS dubbing audio.
- Time-coded custom narration segments with provider voice presets and editable timeline bars for adding explanatory voiceover on top of localization output.
...
- Time-coded narration authoring that lets an operator attach multiple text segments to video time ranges, choose provider-aware voice presets or inherit the configured default voice, inspect and edit computed span/covered/gap/readiness metadata in a compact timeline workbench, preview selected windows against the best available narrated/burned/source video with jump/play-window controls and a visible playhead, export/import explicit narration script packages for workspace reuse, synthesize segments through the existing TTS provider boundary, generate a timed `NARRATION_AUDIO` bed, tune numeric ducking/narration/fade settings, and generate standalone `NARRATED_VIDEO` with original/base audio ducked under narration without replacing generated or reviewed subtitle artifacts. Preview playback is local-only and does not call providers or create artifacts. Gaps are intentional silence; overlaps, invalid windows, and unknown voice presets remain blocked. Voice cloning, uploaded reference audio, voice preview playback, waveform rendering, and multitrack automation curves remain later targets.
