...
- TTS dubbing audio.
- Time-coded custom narration segments with provider voice presets, editable timeline bars, local row editing commands, and metadata-derived waveform overview for adding explanatory voiceover on top of localization output.
...
24. Add time-coded narration rows, choose inherited/default or explicit provider voice presets, duplicate/split/merge/insert local draft rows, move or resize proportional timeline bars, inspect metadata-derived waveform density/gaps, preview and scrub selected windows against narrated/burned/source video with visible playheads, understand gaps as intentional silence, fix overlaps or invalid windows, tune mix settings, generate narration audio/video, run read-only preflight before one-click narration demo render for completed Tears jobs, and download safe narration evidence.
...
- Time-coded narration authoring that lets an operator attach multiple text segments to video time ranges, choose provider-aware voice presets or inherit the configured default voice, duplicate/split/merge/insert local draft rows, inspect and edit computed span/covered/gap/readiness metadata in a compact timeline workbench, inspect metadata-derived waveform density/gap buckets, preview and scrub selected windows against the best available narrated/burned/source video with jump/play-window controls and visible playheads, export/import explicit narration script packages for workspace reuse, synthesize segments through the existing TTS provider boundary, generate a timed `NARRATION_AUDIO` bed, tune numeric ducking/narration/fade settings, and generate standalone `NARRATED_VIDEO` with original/base audio ducked under narration without replacing generated or reviewed subtitle artifacts. Preview playback, waveform scrubbing, and row editing commands are local-only until save and do not call providers or create artifacts. Gaps are intentional silence; overlaps, invalid windows, blank inserted rows, and unknown voice presets remain blocked by validation. Voice cloning, uploaded reference audio, voice preview playback, decoded-audio waveform rendering, and multitrack automation curves remain later targets.
