...
- Time-coded narration authoring that lets an operator attach multiple text segments to video time ranges, synthesize them through the existing TTS provider boundary, and generate separate `NARRATION_AUDIO` plus standalone `NARRATED_VIDEO` without replacing generated or reviewed subtitle artifacts. Audio ducking, multi-track mixing, and waveform editing remain later targets.
