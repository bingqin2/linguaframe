...
- TTS dubbing audio.
- Time-coded custom narration segments with provider voice presets, voice audition, quick script paste import/export, editable timeline bars, local row editing commands, local draft history, per-segment mix overrides, mix automation curves, persistent mix keyframes, and decoded waveform artifacts with metadata fallback for adding explanatory voiceover on top of localization output.
...
- Validate run-day cost safety through the browser `Demo session cost control`, backend cost-control board JSON/Markdown, command-center cost summary, session evidence package cost-control entries, and terminal `scripts/demo/demo-session-cost-control-board.sh` output before another paid demo run.
- Validate demo narration setup through the browser `Narration studio`, `Narration draft history`, `Demo narration preset`, `Render custom narration`, `Render narration demo`, `Render review`, `Playback review`, `Playback resolution`, `Acceptance recovery`, `Narration delivery package`, `Session narration production board`, and `Demo session command center` panels, backend narration-studio aggregate, preset apply/preflight/render, custom saved-workspace preflight/render/report, render-review, playback-review, playback-resolution, recovery-handoff, delivery-package, session production-board, command-center, and session evidence package responses, terminal `scripts/demo/narration-studio.sh`, `scripts/demo/custom-narration-render.sh`, `scripts/demo/narration-demo-preset.sh`, `scripts/demo/narration-demo-render-preflight.sh`, `scripts/demo/narration-demo-render.sh`, `scripts/demo/narration-render-review.sh`, `scripts/demo/narration-playback-review.sh`, `scripts/demo/narration-playback-review-resolution.sh`, `scripts/demo/narration-recovery-handoff.sh`, `scripts/demo/narration-delivery-package.sh`, `scripts/demo/session-narration-production-board.sh`, `scripts/demo/demo-session-command-center.sh`, and `scripts/demo/demo-session-evidence-package.sh`, and optional full Tears custom or preset narration render before narration evidence/review/resolution/recovery/delivery export.
- Validate final demo readiness through the browser `Demo acceptance gate`, browser `Acceptance recovery`, backend gate JSON, backend recovery handoff JSON/Markdown/ZIP, backend narration delivery package JSON/Markdown/ZIP, and terminal full-video `demo-acceptance-gate.json` plus narration recovery handoff, delivery package, and custom narration render handoff output, including the required narration playback resolution check, delivery package readiness evidence, custom render report route, and resolution runbook when narration rows exist.
- Validate offline reviewer handoff through the browser `Demo reviewer workspace`, browser `Demo handoff portal`, backend reviewer/portal JSON/Markdown/ZIP, recovery handoff ZIP, narration delivery JSON/Markdown summaries inside final handoff packages, custom narration render sections and report links, final proof bundle JSON/Markdown summaries that link evidence closure, OpenAI proof, and AI audit routes, and terminal full-video `demo-reviewer-workspace.zip` plus `demo-handoff-portal.zip` output.
- Keep secrets local.
...
- Storage retention.
- Background cleanup.
- Stronger queue and worker recovery. Status: partially implemented with a selected-job stuck recovery cockpit for stale queued/retrying dispatch, stale processing-stage diagnosis, failed-job retry eligibility, confirmed requeue/cancel/retry actions, browser UI, terminal export, metadata-only Markdown, and a run-day demo session recovery board that groups recent jobs into recover-now, watch, review, ready, and no-action rows while linking back to existing recovery and evidence routes. The recovery board is surfaced in the demo session command center, blocks top-level readiness when recover-now rows exist, and is included in the session evidence package as JSON and Markdown.
- Run-day narration production oversight. Status: partially implemented with a metadata-only session narration production board that groups recent jobs into ready, review, render, authoring, blocked, and not-applicable rows while linking back to existing narration scene-board, render-review, playback-resolution, delivery-package, acceptance-gate, and recovery routes. The board is surfaced in the demo session command center, blocks top-level readiness when blocked rows exist, and is included in the session evidence package as JSON and Markdown.
...
21. Open a demo acceptance gate that gives one final `READY`, `ATTENTION`, or `BLOCKED` answer with required checks, warning checks, safe evidence, next action, and safe resolution runbook steps for blocked or warning checks.
...
24. Add time-coded narration rows manually or by quick script paste import, inspect the narration scene board for saved metadata plus local draft text, copy/download the current draft as reusable quick script text, audition configured voice presets, apply a voice to selected/all local draft rows, choose inherited/default or explicit provider voice presets, duplicate/split/merge/insert local draft rows, move or resize proportional timeline bars, use timing assistant actions to close gaps, resolve overlaps, or normalize row order locally, inspect decoded waveform source/buckets with metadata fallback, inspect mix automation curves, add persistent mix keyframes, and apply/clear local overrides, preview and scrub selected windows against narrated/burned/source video with visible playheads, preview selected-row TTS for unsaved text before saving, use in-memory undo/redo/revert before saving, understand gaps as intentional silence, fix overlaps or invalid windows, tune job-level mix settings and selected-row mix overrides, generate narration audio/video, run read-only scene-board/timing/render preflight before one-click narration demo render for completed Tears jobs, inspect the read-only render review cue sheet, and download safe narration evidence.
...
- Time-coded narration authoring that lets an operator attach multiple text segments to video time ranges, inspect a dense scene board with saved metadata, local draft text, segment rail, timeline board, selected-row inspector, readiness checks, and safe actions, paste or export quick script rows in `START-END | VOICE | TEXT` format, audition provider-aware voice presets with custom sample text, apply a voice to selected/all local draft rows, choose provider-aware voice presets or inherit the configured default voice, duplicate/split/merge/insert local draft rows, inspect local draft history with undo/redo/revert and added/removed/timing/text/voice change counts, inspect and edit computed span/covered/gap/readiness metadata in a compact timeline workbench, run local timing assistant fixes for material gaps, overlaps, and row order, run terminal scene-board/timing preflight and decoded waveform export that print only safe metadata, inspect decoded waveform peak/RMS buckets with metadata fallback, reuse persisted waveform artifacts by source hash and bucket count, inspect effective mix automation curves, preview and scrub selected windows against the best available narrated/burned/source video with jump/play-window controls and visible playheads, preview selected-row TTS for unsaved text/voice through a transient provider call, export a read-only render review cue sheet, export/import explicit narration script packages for workspace reuse, run a custom saved-workspace render preflight plus audio/video render without applying a demo preset, export a safe custom render Markdown report, export a read-only narration delivery package with JSON/Markdown/ZIP handoff inventory and generated-media links, synthesize saved segments through the existing TTS provider boundary, generate a timed `NARRATION_AUDIO` bed, tune numeric job-level ducking/narration/fade settings plus optional selected-row overrides, and generate standalone `NARRATED_VIDEO` with original/base audio ducked under narration without replacing generated or reviewed subtitle artifacts. Quick script import/export, scene-board focus/seek actions, timing assistant fixes, render review loading, delivery package loading, voice apply actions, video preview playback, waveform scrubbing, mix automation actions, row editing commands, undo, redo, revert, and segment mix override edits are local/read-only until save and do not call providers. Successful decoded waveform reads create a metadata-only waveform artifact; scene-board, custom render preflight, render review, and delivery package generation do not create artifacts or run FFmpeg; voice audition, segment TTS preview, and explicit custom render may call the configured TTS provider and consume credits, but preview does not save rows, create artifacts, update evidence, write object storage, or generate video. Draft history is in-memory only and resets on workspace reload or successful save response. Gaps are intentional silence; overlaps, invalid windows, invalid mix overrides, blank inserted rows, malformed quick-script rows, and unknown voice presets remain blocked by validation. Voice cloning, uploaded reference audio, and full multitrack editing remain later targets.
