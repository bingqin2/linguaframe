...
OUTPUT_DIR="${LINGUAFRAME_NARRATION_DEMO_PRESET_OUTPUT_DIR:-/tmp/linguaframe-demo/narration-demo-preset}"
REPORT_ONLY="${LINGUAFRAME_NARRATION_DEMO_PRESET_REPORT_ONLY:-false}"

mkdir -p "$OUTPUT_DIR"

CATALOG_PATH="$OUTPUT_DIR/narration-demo-presets.json"
PRESET_PATH="$OUTPUT_DIR/narration-demo-preset.json"
APPLY_PATH="$OUTPUT_DIR/narration-demo-preset-apply.json"
...
if [[ "$REPORT_ONLY" == "true" ]]; then
  echo "Report-only mode: downloaded narration demo preset catalog and recommended preset without changing a job."
  echo "narrationDemoPresetCatalogPath=$CATALOG_PATH"
  exit 0
fi

if [[ -z "$JOB_ID" ]]; then
  echo "Set LINGUAFRAME_DEMO_JOB_ID or pass a job id as the first argument to apply the preset." >&2
  echo "Use LINGUAFRAME_NARRATION_DEMO_PRESET_REPORT_ONLY=true to inspect preset metadata without a job." >&2
  exit 2
fi

apply_narration_demo_preset_json "$BASE_URL" "$JOB_ID" "$resolved_preset_id" "$APPLY_PATH"
print_narration_demo_preset_summary_file "$PRESET_PATH" "$APPLY_PATH"
...
echo "Applied narration demo preset $resolved_preset_id to job $JOB_ID."
echo "Downloaded preset, script package, and narration evidence to $OUTPUT_DIR"
